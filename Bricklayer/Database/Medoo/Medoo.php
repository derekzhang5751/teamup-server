	...
	public function exec($query, $map = [])
	{
		...
		if ($statement)
		{
            ...
            if ($this->trace) {
                ...
                if ($GLOBALS['log'] && $this->timeConsuming > $this->threshold) {
                    $GLOBALS['log']->log('dbtrace', sprintf(' [%.3f] %s', $this->timeConsuming, $this->last()));
                }
            }
			return $statement;
		}
		else
		{
            if ($this->trace) {
                ...
                $this->timeConsuming = $this->timeEnd - $this->timeBegin;
                if ($GLOBALS['log']) {
                    $GLOBALS['log']->log('dbtrace', sprintf(' [%.3f] %s', $this->timeConsuming, $this->error()));
                }
            }
			return false;
		}
